...
from utils import Calibration, draw_path, printf, extract_preds, dir_path, create_gt_distill_roach, load_h5_roach, extract_gt_roach #extract_gt, load_h5
import os
...
def train(run, model, train_loader, val_loader, optimizer, scheduler, recurr_warmup, epoch, 
          log_frequency_steps, train_segment_for_viz, val_segment_for_viz, batch_size, awl, gai_1, gai_2):
    ...
    for tr_it, batch in enumerate(train_loader):
        ...
        loss, loss1, loss2, loss3, loss4, loss5, loss6, loss7, recurr_input, awl_loss = train_batch(run, model, optimizer, stacked_frames, gt_plans, gt_lanelines,
                        gt_leads, gt_leads_prob, gt_desire, traffic_convention, recurr_input, device, timings, should_backprop=should_backprop,
                        awl=awl, gai_1=gai_1, gai_2=gai_2)
        ...
        if should_run_valid:
            ...
            with Timing(timings, 'validate'):
                val_loss = validate(model, val_loader, batch_size, device, awl, gai_1, gai_2)
...
def validate(model, data_loader, batch_size, device, awl, gai_1, gai_2):
    ...
    with torch.no_grad():
        ...
        for val_itr, val_batch in enumerate(data_loader):
            ...
            batch_loss, recurr_input = validate_batch(model, val_stacked_frames, val_plans, val_lanelines, val_leads, val_leads_prob, val_desire, recurr_input, device, awl, gai_1, gai_2)
            val_loss += batch_loss
...
def train_batch(run, model, optimizer, stacked_frames, gt_plans, gt_lanelines, gt_leads, gt_leads_prob, gt_desire, traffic_convention, recurr_input, device, timings, should_backprop=True, awl=None, gai_1=None, gai_2=None):
    ...
    if awl is not None :
        complete_batch_loss = batch_plan_loss + batch_leads_loss + batch_awl_loss
    else :
        complete_batch_loss = batch_plan_loss + batch_plan_pb_loss + batch_leads_loss + batch_leads_pb_loss + batch_pb_lead_loss + batch_desire_pb_loss + batch_desire_st_loss
    ...
    return loss, loss1, loss2, loss3, loss4, loss5, loss6, loss7, recurr_out.detach(), awl_loss


def validate_batch(model, val_stacked_frames, val_plans, val_lanelines, val_leads, val_leads_prob, val_desire, recurr_input, device, awl, gai_1, gai_2):
    ...
    for i in range(seq_len):
        ...
        val_batch_loss += (sg_plan_loss + sg_leads_loss + awl(sg_plan_pb_loss, sg_leads_pb_loss, sg_pb_lead_loss, sg_desire_pb_loss, sg_desire_st_loss))
...
if __name__ == "__main__":
    ...
    checkpoints_dir = './nets/checkpoints_1164'
    result_model_dir = './nets/model_itr_1164'
